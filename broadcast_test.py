...
def inouttests():
    s = TestScenario("Broadcast tests")
    ...
    s.expect(PacketOutputEvent("eth0", testpkt, display=Ethernet), "The Ethernet frame should be forwarded out eth0.")
    s.expect(PacketOutputEvent("eth2", testpkt, display=Ethernet), "The Ethernet frame should be forwarded out eth2.")
    return s
